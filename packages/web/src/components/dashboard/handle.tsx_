...
export type HandlePrivateProps = HandleProps &
  InjectedInputEventsProps & {
...
const enhance = compose<HandleProps, HandleProps>(
  pure,
  withInputEvents,
  withHandlers<HandlePrivateProps, Partial<HandlePrivateProps>>(
    ({onMoveEvents}) => ({
...
export const HandleView = glamorous.button<HandleProps>(
...
export const HandleContainerView = glamorous.div<HandleContainerProps>(
  ({visible}) => ({
    position: 'absolute',
    top: 0,
    left: 0,
