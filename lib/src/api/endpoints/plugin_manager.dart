...
class PluginManagerApi {
    ...
    getAllPlugins(Request request) {
        return pluginManager.readAll().then((data) {
    ...
    putPlugin(Request request) {
        ...
        return pluginManager.save(request.json, plugin).then((data) {
            if (data['err'] != null) {
                return new RestResponse('Update of plugin "$plugin" failed', status: Status.FAIL);
            }

            return new RestResponse('Update of plugin "$plugin" succeeded');
        });
    }
}
